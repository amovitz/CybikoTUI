...
class CybikoCommand(enum.IntEnum):
...
class CybikoSerial:
    ...
    def send_command(self, command: CybikoCommand, data: bytes = b""):
        if len(data) > 255:
    ...
    def clear(self):
        self.send_command(CybikoCommand.CMD_CLEAR)

    def set_cursor(self, row: int, col: int):
        self.send_command(CybikoCommand.CMD_SET_CURSOR, bytes([row, col]))

    def write_text(self, text: str):
        self.send_command(CybikoCommand.CMD_WRITE_TEXT, text.encode("ascii"))

    def put_char(self, row: int, col: int, char: str):
        self.send_command(CybikoCommand.CMD_PUT_CHAR, bytes([row, col, ord(char)]))

    def ping(self, wait_for_pong: bool = True) -> bool:
        self.send_command(CybikoCommand.CMD_PING)
        if not wait_for_pong:
            return True
        frame = self.read_frame()
        return frame is not None and frame[0] == CybikoCommand.EVT_PONG
    ...
    def poll_key(self) -> Optional[Tuple[int, bool]]:
        ...
        if cmd_type == CybikoCommand.EVT_KEY and len(payload) >= 2:
...
if __name__ == "__main__":
    # basic smoke test
    with CybikoSerial(port="/dev/ttyUSB0", baud=9600) as cyb:
        print("pinging...")
        if cyb.ping():
            print("alive!")
        else:
